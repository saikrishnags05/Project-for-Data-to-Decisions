...
# 2. Regression line + confidence intervals
library("ggplot2")
pred.int_ne <- predict(model, interval = "prediction")
mydata1 <- cbind(ag_ne, pred.int_ne)
p <- ggplot(mydata1, aes(y=fit,x=AD_APD,color=AD_year ))+ geom_point()+geom_smooth(method = "lm")+facet_wrap(~AD_year,scales = 'free')
...
* We found out that overall enrollment time is decresed compared to previous years.

* In some facilities the enrollement process is same. 

* Some did not conduct any event for next few years. 

* Overall there is no delay in the registration process compared to previous years. 
...
```{r facility, warning=FALSE, message=FALSE, echo=FALSE}
